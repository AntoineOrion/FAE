...
include(dirname(__FILE__).'/classes/NsFavoriteProduct.php');
...
if (Tools::getValue('action') && Tools::getValue('id_product') && Context::getContext()->cookie->id_customer)
{
	...
	elseif (Tools::getValue('action') == 'add')
	{
		...
		// check if product exists
		if (!Validate::isLoadedObject($product)
			|| NsFavoriteProduct::isCustomerFavoriteProduct((int)Context::getContext()->cookie->id_customer, (int)$product->id))
			die('1');
