...
class Helper
{
    ...
    public static function getTemplatePreviewLink($id)
    {
        ...
        return $previewLink = $context->link->getModuleLink('creativeelements', 'preview', array(
            'id' => $id,
...
{
    $context = Context::getContext();
    $table = _DB_PREFIX_ . 'creativepage_meta';
    $id = (int) $id;
...
function delete_post_meta($id, $key, $lang = 0, $shop = null)
{
    ...
    $shop = $shop === null ? $context->shop->id : (int) $shop;
    return Db::getInstance()->delete('creativepage_meta', "id = $id AND id_lang = $lang AND id_shop = $shop", 1);
}
...
function _x($text, $context, $domain = 'elementor')
{
    // return stripslashes(\Translate::getModuleTranslation($domain, $text, '', null, true));
    return $text;
}
...
{
    // echo Tools::safeOutput(stripslashes(\Translate::getModuleTranslation($domain, $text, '', null, true)));
    echo Tools::safeOutput($text);
}
